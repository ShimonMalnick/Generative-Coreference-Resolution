...
from transformers import AutoConfig, AutoTokenizer, CONFIG_MAPPING, LongformerConfig, RobertaConfig, T5Config, BartConfig
...
def main():
    ...
    if args.model_name_or_path.startswith("t5") or "t5" in args.model_type:
        if args.baseline and "checkpoint" not in args.model_name_or_path:
            print("Init T5 from baseline")
            config = T5Config()
            config.decoder_start_token_id = tokenizer.pad_token_id
            model = T5ForConditionalGeneration(config)
        else:
            print("Init T5 from checkpoint or pretrained model")
            config = T5Config.from_pretrained(args.model_name_or_path)
            model = T5ForConditionalGeneration.from_pretrained(args.model_name_or_path)
        config.dropout_rate = args.dropout_prob
        model.config = config
    elif "bart" in args.model_name_or_path or "bart" in args.model_type:
        if args.baseline and "checkpoint" not in args.model_name_or_path:
            print("Init Bart from baseline")
            config = BartConfig()
            model = BartForConditionalGeneration(config)
        else:
            print("Init Bart from checkpoint or pretrained model")
    ...
    if args.do_train:
        if args.debug:
            train_dataset = get_dataset(args, tokenizer, evaluate=True, file_type="dev")
            exit(0)
        else:
            train_dataset = get_dataset(args, tokenizer, evaluate=False, file_type="train")
        try:
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        result = evaluator.evaluate(model, prefix="final_evaluation", official=True, type="Test")
